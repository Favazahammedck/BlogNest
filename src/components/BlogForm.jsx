...
//     if (currentPost !== null) {
//       updatePost(currentPost, title, content, tags,category);
//       Swal.fire({
//         icon: "success",
...
//       });
//     } else {
//       addPost(title, content, tags,category); // Adds a new post to the context state
//       Swal.fire({
//         icon: "success",
...
const BlogForm = () => {
  const { currentPost, setCurrentPost, addPost, updatePost, posts } = useBlogContext();
  const [title, setTitle] = useState("");
  const [content, setContent] = useState("");
  const [tags, setTags] = useState([]);
  const [tagInput, setTagInput] = useState("");
  ...
  useEffect(() => {
    if (currentPost !== null && currentPost < posts.length) {
      const postToEdit = posts[currentPost];
      setTitle(postToEdit.title);
      setContent(postToEdit.content);
      setTags(postToEdit.tags || []);
      setCategory(postToEdit.category || ""); // Set category if it exists
    }
  }, [currentPost, posts]);
  ...
  const handleSubmit = (e) => {
    ...
    if (currentPost !== null) {
      ...
      addPost(title, content, tags, category); // Add category
      Swal.fire({
        icon: "success",
        title: "Blog Added Successfully",
        showConfirmButton: false,
    ...
    <div className="flex justify-center items-start flex-wrap gap-8 mb-10">
      ...
      >
        <div className="bg-gray-100 p-5 rounded shadow-md w-full max-w-2xl flex flex-col items-start">
          ...
          <div className="border border-gray-300 p-4 rounded bg-white w-full min-h-[300px]">
            <h3 className="text-2xl text-gray-800">{title}</h3>
            <p className="text-lg text-gray-600">{category}</p> {/* Display category */}
            <div className="mt-5 border border-gray-300 p-4 rounded bg-white min-h-[300px]">
              <div className="preview-content" dangerouslySetInnerHTML={{ __html: content }} />
            </div>
          </div>
